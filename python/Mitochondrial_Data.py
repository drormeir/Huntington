...
class Protein_Data:
    def __init__(self, file_name, verbose: bool = False) -> None:
        self.name, _ = os.path.splitext(os.path.basename(file_name))
        if verbose:
            print(f'Reading Mitochondrial protein data: {self.name}')
        csv_data = csv_raw_data(file_name, verbose=False)
        self.df = csv_header_body_2_dataframe(csv_data[0], csv_data[1:], verbose=verbose)
        self.df['Patient_ID'] = Patients_Catalog(verbose=False).find_ID(self.df['Cell_id'])
...
class Morphology_Data:
    def __init__(self, file_name, verbose: bool = False) -> None:
        ...
        csv_data = csv_raw_data(file_name, verbose=False)
        self.df = csv_header_body_2_dataframe(csv_data[0], csv_data[1:], verbose=verbose)
        patients_id = Morphology_Data.group_with_id_2_patient_id(self.df['group_with_id'])
    ...
    @staticmethod
    def group_with_id_2_patient_id(column_group_with_id) -> list[str]:
        fast_mapping = {}
        ...
        return Patients_Catalog(verbose=False).find_ID(patients_id)
